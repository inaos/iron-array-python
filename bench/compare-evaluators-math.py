...
expression = 'sin(x) * sin(x) + cos(x) * cos(x)'
...
if NUMBA_PRECOMP:
    ...
    def poly_numba_prec(x):
        ...
        for i in range(len(x)):
            ...
            y[i] = np.sin(x[i]) * np.sin(x[i]) + np.cos(x[i]) * np.cos(x[i]) 
        return y
...
def do_regular_evaluation():
    ...
    y0 = np.sin(x) * np.sin(x) + np.cos(x) * np.cos(x)
    # print(y0, y0.shape)
    ...
    for i in range(NITER):
        ...
        y1 = np.sin(x) * np.sin(x) + np.cos(x) * np.cos(x)
...
def do_block_evaluation(pshape_):
    ...
    #     ya = (ia.sin(x) * ia.sin(x) + ia.cos(x) * ia.cos(x)).eval(method="iarray_eval", pshape=pshape_, eval_flags=eval_method, **cparams)
