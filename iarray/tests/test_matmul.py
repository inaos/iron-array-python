...
# Matmul
@pytest.mark.parametrize("ashape, achunkshape, ablockshape, abshape, bshape, bchunkshape, bblockshape, bbshape, dtype",
                         [
...
def test_matmul(ashape, achunkshape, ablockshape, abshape, bshape, bchunkshape, bblockshape, bbshape, dtype):
    if achunkshape is None:
        astorage = ia.StorageProperties(backend=ia.BACKEND_PLAINBUFFER)
    else:
        astorage = ia.StorageProperties(achunkshape, ablockshape, enforce_frame=False, backend=ia.BACKEND_BLOSC)
    a = ia.linspace(ia.dtshape(ashape, dtype), -10, 1, storage=astorage)
    an = ia.iarray2numpy(a)

    if bchunkshape is None:
        bstorage = ia.StorageProperties(backend=ia.BACKEND_PLAINBUFFER)
    else:
        bstorage = ia.StorageProperties(bchunkshape, bblockshape, enforce_frame=False, backend=ia.BACKEND_BLOSC)
    b = ia.linspace(ia.dtshape(bshape, dtype), -1, 10, storage=bstorage)
    bn = ia.iarray2numpy(b)

    if abshape is None and bbshape is None:
        cstorage = ia.StorageProperties(backend=ia.BACKEND_PLAINBUFFER)
    else:
        if len(bbshape) == 2:
            ...
            if abshape is not None:
                cchunkshape[0] = abshape[0]
            cchunkshape = tuple(cchunkshape)
        cstorage = ia.StorageProperties(cchunkshape, cchunkshape, backend=ia.BACKEND_BLOSC)
...
@pytest.mark.parametrize(
    ...
    [
        ([100, 100], [20, 20], [10, 12], [20, 40], [70, 90], [23, 32],
         [100, 100], [30, 30], [15, 4], [10, 20], [60, 70], [32, 23], np.float64),
        ([100, 100], None, None, [3, 43], [43, 83], [40, 40],
         [100, 100], None, None, [12, 13], [52, 53], [40, 40], np.float32),
        ([100, 100], [40, 40], [12, 7], [20, 1], [60, 61], [23, 32],
         [100], [44], [12], [3], [63], [32], np.float64),
        ([100, 100], None, None, [32, 32], [52, 62], [20, 30],
         [100], None, None, [12], [42], [30], np.float32),
        ([100, 100], None, None, [43, 23], [93, 93], [50, 70],
         [100, 100], [20, 20], [20, 2], [12, 42], [82, 82], [70, 23], np.float64),
        ([100, 100], [20, 20], [5, 6], [15, 15], [75, 85], [60, 70],
         [100, 100], None, None, [22, 22], [92, 32], [70, 10], np.float32),
        ([100, 100], None, None, [44, 55], [64, 65], [20, 10],
         [100], [44], [31], [12], [22], [10], np.float64),
        ([100, 100], [30, 30], [12, 5], [12, 20], [32, 30], [10, 10],
         [100], None, None, [25], [35], [10], np.float32)
    ])
def test_matmul_slice(ashape, achunkshape, ablockshape, astart, astop, abshape, bshape, bchunkshape, bblockshape,
