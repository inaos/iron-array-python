...
def test_linspace(start, stop, shape, chunks, blocks, dtype, np_dtype, contiguous, urlpath):
    ...
    a = ia.linspace(shape, start, stop, dtype=dtype, np_dtype=np_dtype, chunks=chunks, blocks=blocks, contiguous=contiguous, urlpath=urlpath)
    b = ia.iarray2numpy(a)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (np.datetime64('1964', 'D'), np.datetime64('1965-04', 'D'), [228, 2], [100, 1], [40, 1], np.int64, '<M8[D]',
         True, "test_arange_contiguous.iarr"),
        (np.timedelta64(1234, 'Y'), np.timedelta64(11234, 'Y'), [100, 100], [54, 50], [25, 20], np.int64, '<m8[Y]',
         True, "test_arange_contiguous.iarr")
    ],
)
def test_arange(start, stop, shape, chunks, blocks, dtype, np_dtype, contiguous, urlpath):
    ...
    ia.remove_urlpath(urlpath)
    a = ia.arange(shape, start, stop, step, dtype=dtype, np_dtype=np_dtype, chunks=chunks, blocks=blocks, contiguous=contiguous, urlpath=urlpath)
    b = ia.iarray2numpy(a)
    npdtype = dtype if np_dtype is None else np_dtype
    c = np.arange(start, stop, step, dtype=npdtype).reshape(shape)
    if 'f' in np.dtype(npdtype).str:
        np.testing.assert_almost_equal(b, c)
    else:
...
def test_slice(start, stop, slice, shape, chunks, blocks, dtype, np_dtype, contiguous, urlpath):
    ...
    if type(start) in [np.datetime64, np.timedelta64]:
        ...
        step = (stop - start) / size
    ia.remove_urlpath(urlpath)
    a = ia.arange(shape, start, stop, step, dtype=dtype, np_dtype=np_dtype, chunks=chunks, blocks=blocks, contiguous=contiguous,
                  urlpath=urlpath)
    b = a[slice]
    c = ia.iarray2numpy(b)
    npdtype = dtype if np_dtype is None else np_dtype
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([55, 123, 72], [10, 12, 25], [2, 3, 7], np.bool_, 'u1', True, "test_empty_contiguous.iarr"),
        ([10, 12, 5], [5, 12, 5], [2, 12, 5], np.int64, 'm8[ns]', False, None),
    ],
)
def test_empty(shape, chunks, blocks, dtype, np_dtype, contiguous, urlpath):
    ...
    b = ia.iarray2numpy(a)
    npdtype = dtype if np_dtype is None else np_dtype
    assert b.dtype == np.dtype(npdtype)
...
def test_zeros(shape, chunks, blocks, dtype, np_dtype, contiguous, urlpath):
    ia.remove_urlpath(urlpath)
    a = ia.zeros(shape, dtype=dtype, np_dtype=np_dtype, chunks=chunks, blocks=blocks, contiguous=contiguous, urlpath=urlpath)
    b = ia.iarray2numpy(a)
    npdtype = dtype if np_dtype is None else np_dtype
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param([456, 12, 234], [55, 6, 21], [12, 3, 5], np.float64, None, False, None, marks=pytest.mark.heavy),
...
def test_ones(shape, chunks, blocks, dtype, np_dtype, contiguous, urlpath):
    ia.remove_urlpath(urlpath)
    a = ia.ones(shape, dtype=dtype, np_dtype=np_dtype, chunks=chunks, blocks=blocks, contiguous=contiguous, urlpath=urlpath)
    b = ia.iarray2numpy(a)
    npdtype = dtype if np_dtype is None else np_dtype
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(8.34, [123, 432, 222], [24, 31, 15], [6, 6, 6], np.float64, None, True, None, marks=pytest.mark.heavy),
        (2.00001, [56, 37], [20, 16], [9, 7], np.float32, None, False, "test_full_sparse.iarr"),
        pytest.param(2.00001, [567, 375], [52, 16], [9, 7], np.float32, None, False, "test_full_sparse.iarr", marks=pytest.mark.heavy),
        ...
        (np.datetime64('1929', 'D'), [12, 16], [12, 16], [10, 10], np.int32, '<M8[D]', False, None),
        (np.datetime64('1929', 'ns'), [10, 12, 5], [5, 5, 5], [5, 5, 5], np.int64, np.dtype('datetime64[ns]').str, True,
...
def test_full(fill_value, shape, chunks, blocks, dtype, np_dtype, contiguous, urlpath):
    ia.remove_urlpath(urlpath)
    a = ia.full(shape, fill_value, dtype=dtype, np_dtype=np_dtype, chunks=chunks, blocks=blocks, contiguous=contiguous, urlpath=urlpath)
    b = ia.iarray2numpy(a)
    npdtype = dtype if np_dtype is None else np_dtype
