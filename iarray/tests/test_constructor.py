...
def test_linspace(start, stop, shape, chunkshape, blockshape, dtype):
    if blockshape is None or chunkshape is None:
        ...
        storage = ia.StorageProperties(chunkshape, blockshape, enforce_frame=False, backend=ia.BACKEND_BLOSC)
...
def test_from_file(start, stop, shape, chunkshape, blockshape, dtype, filename):
    ...
    storage = ia.StorageProperties(chunkshape, blockshape, filename, True, ia.BACKEND_BLOSC)
...
def test_ones(shape, chunkshape, blockshape, dtype):
    if blockshape is None:
        ...
        storage = ia.StorageProperties(chunkshape, blockshape, enforce_frame=True, backend=ia.BACKEND_BLOSC)
