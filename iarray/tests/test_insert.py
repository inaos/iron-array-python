...
def test_insert(
    shape, chunks, blocks, data_shape, axis, start, dtype, np_dtype, acontiguous, aurlpath
):
    ...
    if out_dtype not in [np.float64, np.float32]:
        for i in range(len(shape)):
            max *= shape[i]
    a = ia.linspace(0, max, int(np.prod(shape)), shape=shape, cfg=cfg, mode="w", dtype=dtype, np_dtype=np_dtype)
