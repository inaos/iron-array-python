...
def compute_slices(dataset):
    expr = ia.Expr(
        eval_method="iterblock", clevel=CLEVEL, clib=CLIB, nthreads=NTHREADS, blocksize=BLOCKSIZE
    )
    expr.bind("x", dataset)
    expr.compile(sexpr)
...
t1 = time()
print(
    "Time for computing '%s' expression with 3 operands (via numexpr): %.3f" % (sexpr2, (t1 - t0))
)
...
def poly_numba2(x, y, z, w):
    ...
    for i in range(nt):
        for j in range(nx):
            for k in range(ny):
                w[i, j, k] = (
                    (x[i, j, k] - y[i, j, k]) * (z[i, j, k] - 3.0) * (y[i, j, k] - x[i, j, k] - 2)
                )
