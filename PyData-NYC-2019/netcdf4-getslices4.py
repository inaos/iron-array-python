...
t0 = time()
prec1, iobytes1 = concatenate_slices(precipitation)
t1 = time()
print("Time for concatenating %d slices into HDF5 container: %.3f" % (NSLICES, (t1 - t0)))
print("cratio", NSLICES * SLICE_THICKNESS * nx * ny * 4 / len(iobytes1.getvalue()))

np.random.seed(2)
tslice = np.random.choice(nt - NSLICES * SLICE_THICKNESS)
t0 = time()
prec2, iobytes2 = concatenate_slices(precipitation)
t1 = time()
print("Time for concatenating %d slices into HDF5 container: %.3f" % (NSLICES, (t1 - t0)))
print("cratio", NSLICES * SLICE_THICKNESS * nx * ny * 4 / len(iobytes2.getvalue()))

np.random.seed(3)
tslice = np.random.choice(nt - NSLICES * SLICE_THICKNESS)
t0 = time()
prec3, iobytes3 = concatenate_slices(precipitation)
t1 = time()
print("Time for concatenating %d slices into HDF5 container: %.3f" % (NSLICES, (t1 - t0)))
print("cratio", NSLICES * SLICE_THICKNESS * nx * ny * 4 / len(iobytes3.getvalue()))
...
print("Time for computing '%s' expression (via dask + HDF5): %.3f" % (sexpr2, (t1 - t0)))
