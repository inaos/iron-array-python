...
def iarray_eval_disk(expr):
    with ia.config(
        storage=ia.Storage(urlpath="mean-3m.iarr", chunkshape=chunkshape, blockshape=blockshape),
        nthreads=0,
    ) as cfg:
...
def iarray_trans_disk(expr):
    with ia.config(
        storage=ia.Storage(urlpath="trans-3m.iarr", chunkshape=chunkshape, blockshape=blockshape),
        nthreads=0,
    ) as cfg:
        # with ia.config() as cfg:
        expr_val = expr.eval(cfg=cfg)
